...
protected:
	...
	bool read_grammar_file(string input_file);
	bool read_priority_rule();
	bool read_production();
	bool translate_action();
	bool output_action(string input_file);
	bool read_one_char();
	void retract_one_char();
	bool isLetterOrDigit(char ch);
	bool isLegalLetter(char ch);
	
	
	bool init_lr1_dfa();
	void construct_lr1_table();
	void output_table();
	LR1State Closure(LR1State X);
	LR1State GOTO(LR1State lr1state, unsigned int edge);
	set<ElementType> First(ElementType X);
	set<ElementType> Follow(ElementType X);
	ACTION_TYPE conflict_solve(ElementType shiftOP, ElementType reduceOP);
	//// help function
...
private:
	...
	std::map<ElementType, bool> _nonterminalNullable;//存所有空产生式的左端
